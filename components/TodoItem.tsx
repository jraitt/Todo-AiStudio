...
const TodoItem: React.FC<TodoItemProps> = ({ todo, onToggle, onDelete, onEdit }) => {
  ...
  return (
    <div className="p-4 bg-white rounded-xl shadow-sm border border-gray-200/80 transition-all duration-300 hover:shadow-md hover:border-gray-300/80">
      {/* Row 1: Checkbox + Todo Name */}
      <div className="flex items-center gap-3 mb-3">
        ...
        <p className={`text-base font-medium text-gray-800 flex-grow ${todo.completed ? 'line-through text-gray-400' : ''}`}>
          {todo.text}
        </p>
      </div>

      {/* Row 2: Priority + Project + Edit & Delete Buttons */}
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2 text-gray-400">
          <button onClick={() => onEdit(todo.id)} className="p-1 hover:text-blue-500 hover:bg-blue-50 rounded-md transition-colors duration-200">
            <EditIcon className="w-5 h-5" />
          </button>
          <button onClick={() => onDelete(todo.id)} className="p-1 hover:text-red-500 hover:bg-red-50 rounded-md transition-colors duration-200">
            <TrashIcon className="w-5 h-5" />
          </button>
        </div>
      </div>

      {/* Row 3: Created Date + Due Date */}
